...
def start():
    while True:
        ...
        match choice:
            ...
            case 3:
                view.show_book(model.phone_book, text.empty_phone_book)
            case 4:
                contact = view.input_contact(text.new_contact)
                model.add_contact(contact)
                view.print_massage(text.conntact_save_successful(contact[0], text.contact_operation[0]))
            case 5:
            ...
            case 6:
                if search_block(text.for_edit):
                    ...
                    view.print_massage(text.conntact_save_successful(name, text.contact_operation[1]))
            case 7:
                if search_block(text.for_delete):
                    del_id = int(view.input_request(text.input_del_id))
                    name = model.del_contact(del_id[0])
                    view.print_massage(text.conntact_save_successful(name, text.contact_operation[2]))
            case 8:
